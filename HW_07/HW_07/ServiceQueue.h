	...
	void update(int clock)
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {
				// FIXME done remove plane from the service queue
				the_queue.pop();

				// FIXME: done update the enter_departure_time attribute for the plane
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				// FIXME:done calculate the wait time of the plane in the landing queue
				int wait_landingtime = clock - plane->arrival_time;

				// FIXME:done update total_wait and num_served for the landing queue
				landing_queue->total_wait = landing_queue->total_wait + wait_landingtime;

				landing_queue ->num_served++;


				// FIXME:done  update the start_service_time attribute for the plane
				plane -> start_service_time = clock;

				/* FIXME: done compute a random service time for the plane between min_service_time and max_service_time
